...
src/
  ...
  base/input-element.ts          Form-associated base for form controls (+ input-common.css)
...
- Leading/trailing icons are `icon-start` / `icon-end` (an `IconName` from the set); `icon-only` is the boolean for a square icon button. A slot (`icon`) is added only where arbitrary markup — an image, a logo — is also wanted (e.g. `moz-page-header`). On form controls `icon-start` is the in-field icon; a `label-icon` sits in the label (the shared `labelContent` helper renders it for both the control's built-in label and `moz-label`).
- Events are named `moz-<component>:<verb>` and dispatched `{ bubbles: true, composed: true }` (plus `cancelable` for a preventable action). Any payload gets an exported `<Name>Detail` interface. Present tense is the request (`:dismiss`), past tense the completed notification (`:dismissed`). Form controls are the deliberate exception: they re-emit the platform's `input` / `change` (composed) so they behave like native fields for consumers and form libraries.
- Form controls extend `MozBaseInputElement` (`src/base/input-element.ts`, styled by `input-common.css`), not `MozLitElement` directly. It provides `ElementInternals` form participation, the label/description scaffolding, validation mirroring, and the `inputLayout` modes; a subclass only implements `inputTemplate()`. See `moz-input-text`.
- Shared render/behavior helpers live in `src/base/`: `slots.ts` (`slotHasContent`), `roving.ts` (`rovingIndex` for arrow-key groups), `icon-button.ts` (`iconButton` for close/back/clear controls), and `label-content.ts` (`labelContent` for the shared label icon/text/required markup). Reuse them rather than re-implementing.
